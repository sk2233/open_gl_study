...
func (m *MainGame) Init(window *glfw.Window) {
	vs := utils.LoadObj("rabbit_cocos/test2/reisen.obj")
	m.obj = frame.NewObj(vs, int32(len(vs)/8))
	m.obj.VertexAttr(0, 3, 4*8, 0)
	m.obj.VertexAttr(1, 2, 4*8, 4*3)
	m.obj.BindTexture(gl.TEXTURE0, utils.LoadTexture("rabbit_cocos/test2/reisen.png"))

	m.shader = frame.NewShader("rabbit_cocos/test2/test")
	m.shader.Use()
	p := utils.GetDefaultPerspective()
	m.shader.SetMat4("uProjection", &p[0])

	m.effect = frame.NewPostEffect(1280, 720, 1)
	m.effectShader = frame.NewShader("rabbit_cocos/test2/effect")
}
