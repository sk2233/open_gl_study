...
func main() {
	...
	for !window.ShouldClose() {
		...
		gl.UseProgram(program)
		gl.Uniform4f(ourColor, r*2, 0, 1, 1)
		gl.BindVertexArray(vao)

		model := mgl32.Translate3D(0.1, 0, 0).Mul4(mgl32.HomogRotate3DY(r)) // 模型矩阵 放到世界空间
		view := mgl32.Translate3D(0, 0, -5)                                 // 视图矩阵 变化相机
		view = mgl32.LookAt(0, 3, 3, 0, 0, 0, 0, 1, 0)
		// 正交/透视矩阵   实现相机效果
		projection := mgl32.Perspective(45, WindowWidth/WindowHeight, 0.1, 100)
		res := projection.Mul4(view).Mul4(model) // 最终中间值还需要物体应用

		gl.UniformMatrix4fv(transform, 1, false, &res[0])
